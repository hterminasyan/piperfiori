...
module.exports = function(grunt) {
            ...
            copyTmpToDist: {
                files: [
                    ...
                    },
                    {
                        ...
                        filter: function(filepath) {
                            // prevent js and css files and contents of webapp/test from being copied
							return !filepath.match(new RegExp("(" + tmpDir + "(\\/|\\\\)test|${tmpDir}(\\/|\\\\)localService|\\.js$|\\.css$|\\test.html$)", "gi"));                        }
                    }]
            }
		},
        mkdir: {
        ...
        "codevalidation": {
            "di.code-validation.js": {
                options: {
                    ...
                    pathPrefix: path.join(path.sep, '<%= dir.root %>'),
	...
	grunt.registerTask('devx-updateNeoApp', 'Update neo-app for build', function() {
        ...
        if (bNeoAppExists){
			...
			grunt.file.write(grunt.config.data.dir.dist + "/neo-app.json", JSON.stringify(oNeoAppJsonContent));
        }
	});
	...
	grunt.registerTask('cleanTmp', 'Clean temp data created during the build', function() {
 	...
 	// Build task
	grunt.registerTask('build', ["copy:copyToDbg", "openui5_preload:preloadDbg", "copy:copyToTmp", "uglify:uglifyTmp", "cssmin", "openui5_preload:preloadTmp", "copy:copyDbgToDist", "uglify:uglifyPreload", "copy:copyTmpToDist", 'devx-updateNeoApp', 'devx-updateManifest', 'cleanTmp']);
